...
(*     red_tt p q -> p ⪯ₜᴛb q. *)
(* Proof. *)
(*   intros X Y p q mp' Hsp (f & Hf). *)
(*   pose (qus := fun x => projT1 (f x)). *)
(*   pose (tb := fun x L => ext_eval (projT2 (f x)) L). *)
...
(*                                          (tb x L <-> b = true)) _) *)
(*     (fun g x => bind (part_map g (qus x)) (fun L => ret (eval_tt (projT2 (f x)) L))) *)
(*     (fun x => qus x)). *)
...
(*       * unfold tb. rewrite truthtable_extension. reflexivity. *)
(*     + intros (L & HL1 & HL2). *)
...
(*         eapply ret_hasvalue'. *)
(*         eapply Bool.eq_true_iff_eq. red in Hf. unfold reflects in Hf. *)
(*         rewrite <- HL2. unfold tb. rewrite <- truthtable_extension. *)
(*         eapply pcomputes_part_map in Hgl. 2: eauto. *)
(*         eapply PropExt'. f_equal.  *)
(*         clear - Hl Hgl. revert Hgl. generalize (qus x). *)
(*         induction Hl; cbn; intros L Hgl; f_equal; inv Hgl. *)
...
(*   - cbn. eapply FunRel_ext. intros x. cbn. red in Hf. unfold reflects in Hf. *)
(*     intros b. split. *)
(*     + rewrite reflects_iff. unfold reflects.  *)
(*       intros H. exists (map q (qus x)). *)
(*       split. *)
(*       * admit. *)
(*       * unfold tb. rewrite <- H. clear b H. split. *)
(*         -- admit. *)
(*         --  *)

(*       2: firstorder. *)
(*       eapply Forall2_fmap_r, Forall_Forall2, Forall_forall. *)
...
(* Proof. *)
(*   intros X Y p q mp' Hsp (f & Hf).  *)
(*   pose proof (mp := MP_to_MP_partial mp'). red in Hf. *)
(*   unshelve eapply (mktotalbTuring  *)
(*     (fun R : FunRel Y bool => *)
(*        @Build_FunRel _ _ (fun x b => exists L, List.Forall2 (fun x (b : bool) => if b then the_rel R x true else the_rel R x false) (projT1 (f x)) L /\ eval_tt (projT2 (f x)) L = b) *)
(*                      _) *)
(*     (fun g x => bind (part_map g (projT1 (f x))) (fun L => ret (eval_tt (projT2 (f x)) L))) *)
(*     (fun x => projT1 (f x))). *)
(*   - (* intros x b1 b2 [L1 [H1 H1']] [L2 [H2 H2']]. *) *)
(*     (* cstart. *) *)
(*     (* enough (~~ (b1 = true <-> b2 = true)). { cunwrap. now cprove eapply Bool.eq_true_iff_eq. } *) *)
...
(*     admit. *)
(*   - (* intros g R Hg x. intros b. split; cbn. *) *)
(*     (* + intros (L & ? % (pcomputes_part_map _ _ _ Hg) & <- % ret_hasvalue_inv) % bind_hasvalue. intros *) *)
(*     (*   exists (map (eq true) L). split. *) *)
(*     (*   * eapply Forall2_fmap_r, Forall2_impl; eauto. *) *)
(*     (*     cbn. clear - H0. intros ? [] ?; firstorder. (* enough (true = false) by congruence. all: eapply R; eauto. *) *) *)
(*     (*   * eapply Hf. *) *)
...
(*     (*     eapply Forall2_transitive; eauto. *) *)
(*     (*     cbn. unfold reflects. intros. *) *)
(*     (*     destruct z; try now firstorder. *) admit. *)
(*   - cbn. intros x R R' Hcont b (L & H1 & H2). exists L; split; eauto. *)
(*     revert Hcont. clear H2. *)
(*     induction H1. *)
(*     * econstructor. *)
(*     * intros. econstructor; eauto. destruct y; eapply Hcont; eauto. *)
(*   - cbn. eapply FunRel_ext. intros x. cbn. *)
(*     intros b. split. *)
(*     + rewrite reflects_iff. unfold reflects. rewrite H *)
(*       intros H. exists (map q (qus x)). *)
(*       split. 2: firstorder. *)
(*       eapply Forall2_fmap_r, Forall_Forall2, Forall_forall. *)
(*       intros y. cbn. intros. firstorder. *)
