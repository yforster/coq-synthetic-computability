...
Definition EAB := ∑ e : nat -> (nat -> option nat), forall p : nat -> Prop, enumerable p <-> exists c, enumerator (e c) p.

Lemma EAB_EA : EAB -> EA.
Proof.
  intros [e H].
  exists e.
  intros f. specialize (H (fun x => exists n, f n = Some x)) as [[c Hc] _].
  - exists f. red. reflexivity.
  - exists c. cbn. red in Hc. now setoid_rewrite Hc.
Qed.
...
Proof.
  ...
  exists e.
  intros p. split.
  - intros [f Hf]. destruct (H f) as [c Hc]. exists c.
    red. cbn in Hc. setoid_rewrite Hc. eapply Hf.
  - intros [c Hc]. now exists (e c).
Qed.
...
Lemma EPF_to_CTP `{partiality} :
  EPF -> ∑ m : model_of_computation, CTP.
Proof.
  move => [e EPF].
  unshelve eexists.
...
Proof.
  rewrite /ofenum.
  move => H. eapply bind_hasvalue in H as [n [[H1 _] % mu_tot_hasvalue H2]].
  destruct (g n) as [[x' y']|] eqn:E.
